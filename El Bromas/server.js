...
fs.readFileSync=function(file,enc){
 ...
 if(typeof out==='string'&&/server-base\.js$/.test(name)){
  ...
  // Server owns the live NPC snapshot. Clients only mirror it; they never create the
  // initial 7-NPC round just because they connected.
  ...
  // Every connection receives an explicit role. Only the first player of a fresh world
  // gets permission to run the original game's initial local wave generator.
  ...
  x=x.replace(/players\.set\(s\.id,p\);if\(!npcSource\)npcSource=s\.id;/,
   "players.set(s.id,p);if(canSpawnInitial){npcSource=s.id;worldNeedsBootstrap=false}else if(!npcSource)npcSource=s.id;");
  x=x.replace(/s\.emit\('room:welcome',\{id:s\.id,npcSource:s\.id===npcSource,\.\.\.syncPack\(\)\}\);/,
   "s.emit('room:welcome',{id:s.id,npcSource:s.id===npcSource,canSpawnInitial,authoritativeJoin:true,npcJoinSnapshot:{round,alive:gameState.alive,seq:npcSnapshot.seq,serverTime:npcSnapshot.serverTime,items:npcSnapshot.items},...syncPack()});");
  x=x.replace(/s\.on\('room:join',d=>\{p\.name=clean\(d\?\.name\);p\.round=round;s\.emit\('room:sync',syncPack\(\)\)\}\);/,
   "s.on('room:join',d=>{p.name=clean(d?.name);p.round=round;s.emit('room:sync',{...syncPack(),npcSource:s.id===npcSource,canSpawnInitial:wasEmpty&&round===0&&gameState.alive===0&&npcSnapshot.items.length===0,authoritativeJoin:true,npcJoinSnapshot:{round,alive:gameState.alive,seq:npcSnapshot.seq,serverTime:npcSnapshot.serverTime,items:npcSnapshot.items}});s.emit('npc:join:sync',{round,alive:gameState.alive,seq:npcSnapshot.seq,serverTime:npcSnapshot.serverTime,items:npcSnapshot.items})});");
  // Keep normal room-sync emissions authoritative too.
  x=x.replace(/s\.emit\('room:sync',syncPack\(\)\)/g,
   "s.emit('room:sync',{...syncPack(),npcSource:s.id===npcSource,canSpawnInitial:false,authoritativeJoin:true,npcJoinSnapshot:{round,alive:gameState.alive,seq:npcSnapshot.seq,serverTime:npcSnapshot.serverTime,items:npcSnapshot.items}})");

  // Only the authoritative NPC publisher may update the server roster.
  ...
  // If the current authority leaves while players remain, immediately elect another
  // authority without resetting the world or granting it a fresh round spawn.
  x=x.replace(/s\.on\('disconnect',\(\)=>\{/g,
   "s.on('disconnect',()=>{if(s.id===npcSource){npcSource=null;const next=players.values().next().value;if(next){npcSource=next.id;io.to(next.id).emit('room:role',{npcSource:true,canSpawnInitial:false,authoritativeJoin:true,npcJoinSnapshot:{round,alive:gameState.alive,seq:npcSnapshot.seq,serverTime:npcSnapshot.serverTime,items:npcSnapshot.items}})}};");
 ...
 if(typeof out==='string'&&/online-final14\.js$/.test(name)){
  let x=out;
  // A re-elected authority can still render the server snapshot and republish it;
  // it is not allowed to manufacture a new round on its own.
 ...
 if(typeof out==='string'&&/online-final25\.js$/.test(name)){
  let x=out;
  // Distinguish authority from permission to perform the original game's initial spawn.
  ...
  x=x.replace(/const badText=t=>\/evento\\s\+protesta\\s\+vand\[aá\]listica\|\^oleada\\s\*\[:#-\]\?\\s\*\\d\*\|\^listo\\.?\$\/i,/,
   "const badText=t=>/(?:^|@\\s*)oleadas?\\s*[:#-]?\\s*\\d*\\s*$|evento\\s+protesta\\s+vand[aá]listica|^listo\\.?$/i,");
  ...
  x=x.replace(/serverNpc=Array\.isArray\(m\?\.items\)\?m\.items:\[\];synced=true;applyAuthoritativeState\(\);removeLateJoinNpcs\(\);/,
   "serverNpc=Array.isArray(m?.items)?m.items:[];synced=true;applyAuthoritativeState();removeLateJoinNpcs();unfreezeSource();");
  ...
  x=x.replace(/if\(typeof d\?\.npcSource==='boolean'\)source=!!d\.npcSource;const g=d\?\.gameState\|\|\{\};/,
   "if(typeof d?.npcSource==='boolean')source=!!d.npcSource;if(typeof d?.canSpawnInitial==='boolean')allowLocalSpawn=!!d.canSpawnInitial;const g=d?.gameState||{};");
  x=x.replace(/if\(source\)unfreezeSource\(\);else freezeLocalRound\(\)/g,"if(allowLocalSpawn)unfreezeSource();else freezeLocalRound()");
