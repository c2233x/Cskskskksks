...
fs.readFileSync=function(file,enc){
 ...
 if(typeof out==='string'&&/server-base\.js$/.test(name)){
  let x=out;
  x=x.replace(/let round=0,npcSource=null,seq=0;/,"let round=0,npcSource=null,seq=0,worldNeedsBootstrap=true,emptyResetTimer=null,initialNpcsCreated=false;\nconst cancelEmptyReset=()=>{if(emptyResetTimer){clearTimeout(emptyResetTimer);emptyResetTimer=null}};\nconst scheduleEmptyReset=()=>{if(emptyResetTimer||players.size)return;emptyResetTimer=setTimeout(()=>{emptyResetTimer=null;if(players.size)return;round=0;npcSource=null;worldNeedsBootstrap=true;initialNpcsCreated=false;npcSnapshot={seq:0,serverTime:Date.now(),round:0,items:[]};worldSnapshot={seq:0,serverTime:Date.now(),round:0,items:[]};fxSnapshot={seq:0,serverTime:Date.now(),items:[]};worldUi={seq:0,serverTime:Date.now(),items:[]};worldVisual={seq:0,serverTime:Date.now(),bg:null,lights:[]};worldTime=0;gameState={seq:0,serverTime:Date.now(),round:0,alive:0,deaths:0,status:'',mission:false,missionType:'',posters:0,cinematic:false,cinematicPhase:0,cinematicTimer:0,boss:null,cam:null,cinematicEpoch:0,cinematicState:null};carOwners.clear();carPassengers.clear();carRadios.clear();fxPlayers.clear();log('RESET','Mundo reiniciado tras 60s sin jugadores')},60000)};\nconst bootstrapServerNpcs=()=>{if(initialNpcsCreated||npcSnapshot.items.length)return;initialNpcsCreated=true;npcSnapshot={seq:++seq,serverTime:Date.now(),round:1,items:Array.from({length:7},(_,i)=>({id:'server:pene:'+i,type:'pene',x:(i-3)*4,y:0,z:(i%2?4:-4),rx:0,ry:0,rz:0,hp:100,maxHp:100,walk:0,attack:false,protester:false,leap:false,leapPhase:0,fly:0,giant:false}))};round=1;gameState.round=1;gameState.alive=7;worldNeedsBootstrap=false;log('NPC','Creados los 7 NPC iniciales por el servidor')};\n");
  x=x.replace(/io\.on\('connection',s=>\{const p=\{/,"io.on('connection',s=>{const wasEmpty=players.size===0;cancelEmptyReset();const p={");
  x=x.replace(/players\.set\(s\.id,p\);if\(!npcSource\)npcSource=s\.id;/,"players.set(s.id,p);if(wasEmpty&&worldNeedsBootstrap&&!initialNpcsCreated)bootstrapServerNpcs();if(!npcSource)npcSource=s.id;");
