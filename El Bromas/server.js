...
const SYNC='<script src="/socket.io/socket.io.js"></script><script src="/online-sync.js"></script>';
const MAX_PLAYERS=64,players=new Map(),chatHistory=[];let hostId=null,world=null;
...
function setWorld(d){if(!d||!Array.isArray(d.npcs))return;world={time:Date.now(),round:Math.max(0,Math.floor(num(d.round,d.wave))),npcs:d.npcs.slice(0,300).map(n=>({id:clean(n?.id).slice(0,64),type:clean(n?.type).slice(0,20),x:num(n?.x),y:num(n?.y),z:num(n?.z),rx:num(n?.rx),ry:num(n?.ry),rz:num(n?.rz),hp:Math.max(0,num(n?.hp)),maxHp:Math.max(1,num(n?.maxHp,1)),scale:Math.max(.05,num(n?.scale,1)),giant:!!n?.giant,walk:num(n?.walk),leap:!!n?.leap}))}}
function serveGame(_q,r){try{let h=fs.readFileSync(GAME_FILE,'utf8');h=h.replace(/<script[^>]*src=["']\/socket\.io\/socket\.io\.js["'][^>]*><\/script>/gi,'').replace(/<script[^>]*src=["']\/online-sync\.js["'][^>]*><\/script>/gi,'');h=h.replace(/<\/body>/i,SYNC+'</body>');r.set('Cache-Control','no-store,no-cache,must-revalidate,proxy-revalidate');r.type('html').send(h)}catch(e){console.error(e);r.status(500).send('No se pudo cargar el juego')}}
io.on('connection',s=>{
 if(players.size>=MAX_PLAYERS){s.emit('server:full');return s.disconnect(true)}
 if(!hostId)hostId=s.id;
 const p={id:s.id,name:'Jugador',x:0,y:0,z:0,rx:0,ry:0,rz:0,hp:100,aura:0,kills:0,round:0,dead:false,moving:false,running:false,jumping:false,flying:false,attacking:false,special:false,attackPhase:0,specialPhase:0,walkCycle:0,pose:{},lastUpdate:Date.now()};players.set(s.id,p);
 s.emit('room:welcome',{id:s.id,host:s.id===hostId,players:pub(),world});s.emit('chat:history',chatHistory.slice(-50));broadcast();
 s.on('room:join',d=>{p.name=clean(d?.name);p.round=world?world.round:num(d?.round);p.kills=num(d?.kills);s.emit('room:welcome',{id:s.id,host:s.id===hostId,players:pub(),world});s.broadcast.emit('player:joined',{id:s.id,name:p.name});broadcast()});
 ...
 s.on('world:state',d=>{if(s.id===hostId&&d&&Array.isArray(d.npcs)){setWorld(d);io.emit('world:state',world);io.emit('round:state',{round:world.round,time:world.time})}});
 ...
 s.on('disconnect',reason=>{const was=s.id===hostId;players.delete(s.id);s.broadcast.emit('player:left',{id:s.id,name:p.name,reason});if(was){hostId=players.keys().next().value||null;if(hostId)io.to(hostId).emit('room:host',true)}broadcast()});
});
...
app.get('/',serveGame);app.get('/Salva%20a%20cornatan%20%F0%9F%91%88.html',serveGame);app.get('/Salva a cornatan 👈.html',serveGame);app.use(express.static(__dirname,{maxAge:'1h'}));
