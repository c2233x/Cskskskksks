...
fs.readFileSync=function(file,enc){
 ...
 if(typeof out==='string'&&/server-base\.js$/.test(name)){
  ...
  x=x.replace(/let round=0,npcSource=null,seq=0;/,
   "let round=0,npcSource=null,seq=0,worldNeedsBootstrap=true,emptyResetTimer=null;\n"+
  ...
  x=x.replace(/io\.on\('connection',s=>\{/,"io.on('connection',s=>{");
  ...
  x=x.replace(/io\.on\('connection',s=>\{/,"io.on('connection',s=>{s.on('combat:attack',d=>{const id=String(d?.id||'');const damage=Math.max(1,Math.min(100,Number(d?.damage)||10));const items=Array.isArray(npcSnapshot.items)?npcSnapshot.items.slice():[];const i=items.findIndex(n=>String(n.id)===id);if(i<0)return;items[i]={...items[i],hp:Math.max(0,Number(items[i].hp)||0-damage)};if(items[i].hp<=0)items.splice(i,1);npcSnapshot={...npcSnapshot,seq:++seq,serverTime:Date.now(),items};gameState.alive=items.length;io.emit('npc:snapshot',npcSnapshot);io.emit('game:state',gameState)});");
  return x;
 }
